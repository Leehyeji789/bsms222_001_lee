...
ifelse(a > 0, 1/a, NA)
```
The function is particularly useful because it works on vectors. It examines each entry of the logical vector and returns elements from the vector provided in the second argument, if the entry is `TRUE`, or elements form the vector provided in the third argument, if the entry is `FALSE`.
```{r}
a <- c(0,1,2,-4,5)
result <- ifelse(a > 0, 1/a, NA)
```
This table hleps us see what happened:
```{r}
is_a_positive <- a > 0
answer1 <- 1/a
answer2 <- NA
data.frame(a, is_a_positive, answer1, answer2, result)
```
...
If we want to be absoultely sure that we use the dplyr `filter`, we can use
```{r}
dplyr::filter
```
...
For more on this more advanced topic we recommned the R packages book.
...
```
- *"Not all Positives" because there is **-3**.*

---

2. Which of the following expression is always `FALSE` when at least one entry of a logical vector `x` is TRUE?
...
x <- sapply(n, function(n)(n*n+1))

y <- vector("numeric", length=100)
y <- sapply(n, function(n)(100*n))
...
- *3. Variables are not affected from variables used for description the function.*
...
```{r}
n <- 1:25
s_n <- purrr::map_dbl(n, compute_s_n)
s_n
```

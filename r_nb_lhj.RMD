---
title: "R Notebook_Chapter 2"
---

## 2.2 The very basics

Before we started with the motivating dataset, we need to cover the very basics of R.

### 2.2.1. Objects

Suppose a high school students asks us for help solving several quadratic equations of the form **ax^2 + bx + c = 0**. The quadratic formula gives us the solutions: (formula)
...
which stores the values for later use. We use `<-` to assign values to variables. 
We can also assign values using `=` instead of `<-`, but we recommend against using `=` to avoid confusion. 

Copy and paste the code above into your console to define the the three variables. Note that R does not print anything when we make this assignment. This means the objects were defined successfully. Had you made a mistake, you would have received an error message.

To see the value stored in a variables, we simply ask R to evaluate `a` and it shows the stored value:
```{r}
a
```
...
Once you define variables, the data analysis process can usually be described as a series of *functions* applied to the data. R includes several predefined and most of the analysis pipelines we construct make extensive use of these.

We already used the `install.packages`, `library`, and `ls` functions. We also used the function `sqrt` to solve the quadratic equation above. There are many more prebuilt functions and even more can be added through the packages. Theses functions do not appear in the workspace because you did not define them, but they are available for immediate use.
...
log(a)
```
You can find out what the function expects and what it does by reviewing the very useful manuals included in R. You can get help by using the `help` functions like this:
```{r}
help("log")
```
...
?log
```
The help page will show you what arguments the functions is expecting. For example, `log` needs `x` and `base` to run. However, some arguments are required and others optional. You can determine which arguments are optional by noting in the help document that a default value is assigned with `=`. Defining theses is optional. For example, the base of the function `log` defaults to `base = exp(1)` making `log` the natural log by default.
...
log(x = 8, base = 2)
```
The above code works, but we can ourselves some typing: if no arguments name is used, R assumes you are entering arguments in the order shown in the help file or by `args`. So by not using the names, it assumes the arguments are `x` followed by `base`:
```{r}
log(8,2)
```
To specify arguments, we must use `=`, and cannot use `<-`.

There are some exceptions to the rule that functions need the parentheses to be evaluated. Among theses, the most commonly used are the arithmetic and relational operators. For example:
```{r}
2 ^ 3
```
...
There are several datasets that are included for user to practice and test out functions. You can see all the available datasets by typing:
```{r}
data()
```
...
A nice convention to follow is to use meaningful words that describe what is stored, use only lowercase, and use underscores as a substitute for spaces. For the quadratic equations, we could use something like this:
```{r}
...
(-b - sqrt(b^2 - 4*a*c)) / (2*a)
```
By creating and saving a script with the code above, we would not need to retype everything each time and, instead, simply change the variable name. Try writing the script above into an editor and notice how easy it is to change the variables and receive an answer.
...
1. What is the sum of the first 100 positive integers? The formula for the sum of integers 1 through **n** is **n(n+1)/2**. Define **n = 100** and then use R to compute the sum of **1** through **100** using the formula. What is sum?
...
b. ***<u>`seq` creates a list of numbers and `sum` adds them up.</u>***  
...
names(murders)
```
It is important to know that the other of the entries in `murders$population` preserves the order of the rows in our data table. This will later permit us to manipulate one variable based on the results of another. For example, we will be able to order the state names by the number of murders.
...
Another important type of vectors are *logical vectors*. These must be either `TURE` or `FALSE`.
```{r}
z <- 3 == 2
z
```
...
We discuss more important feature of vectors after the nest set of exercises.

**Advanced**: Mathematically, the values in `pop` are integers and there is an integer class in R. However, by default, numbers are assigned class numeric even when they are round integers. Fore example, `class(1)` returns numeric. You can turn into class integer with the `as.integer()` function or by adding an `L` like this: `1L`. Note the class by typing: `class(1L)`
```{r}
class(1)
```
```{r}
...
In the `murders` dataset, we might expect the region to also be a character vectors. However, it is not:
```{r}
...
Note that the levels have an order that is different from the order of appearance in the factor object. The default in R is the levels to follow alphabetical order. However, often we want the levels to follow a different order. You can specify an order through the `levels` argument when creating the factor with the `factor` function. For example, in the murders dataset regions are ordered form east to west. The function `reorder` lets us change the order of the levels of a factor variable based on a summary computed on a numeric vector. We will demonstrate this with a simple example, and will see more advanced ones in the Data Visualization part of the book.
...
**Warning** : Factors can be a source of confusion since sometimes they behave like characters and sometimes do not. As a result, confusing factors and characters are a common source of bugs.
...
record[["student_id"]]
```
You should get used to the fact in R, there are often several ways to do the same thing, such as accessing entries.
...
record2
```
If a list does not have names, you cannot extract the elements with `$`, but you can still use the brackets method and instead providing the variable name, you provide the list index, like this:
```{r}
record2[[1]]
```
We won't using lists until later, but you might encounter one in your own exploration of R. For this reason, we show you some basics here.
...
Use the function `str` to examine the structure of the `murder` object. Which of the following best describes the variables represented in this data frame?
...
c. *<u>The state name, the abbreviation of the state name, the state's region, and the state's population and the total number of murder for 2010.</u>*
...
4. Now use the square brackets to extract the state abbreviations and assign them to object `b`. Use the `identical` function to determine if `a` and `b` are the same.
...
In R, the most basic objects available to store are vectors. As we have seen, complex datasets can usually be broken down into components that are vectors. For example, in a data frame, each column is a vector. Here we learn more about this important class.

### 2.6.1 Creating vectors

We can create vectors using the function `c`, which stands for *concatenate*. We can use `c` to concatenate entries in the following way:
```{r}
codes <- c(380, 124, 818)
codes
```
...
codes
```
There is no difference between this function call and previous one. This is one of the many ways in which R is quirky compared to other languages.
...
seq(1, 10)
```
The first argument defines the start, and the second defines the end which is included. The default is to go up increment of 1, but a third argument lets us tell it how much to jump by:
```{r}
seq(1, 10, 2)
```
...
In general, *coercion* is an attempt by R to be flexible with data types. When an entry does not match the expected, some the prebuilt R functions try to guess what was before throwing an error. This can also lead to confusion. Failing to understand coercion can drive programmers crazy when attempting to code in R since it behaves quite differently from most other languages in this regard. Let's learn about it with some examples.

We said vectors must be all the same type. So if we try to combine, say, numbers and characters, you might expect an error:
```{r}
x <- c(1, "canada", 3)
```
...
11. The class of `class(a<-1)` is numeric, not integer. R defaults to numeric and to force to integer, you need to add the letter `L`. Confirm that the class of `1L` is integer.
```{r}
class(1L)
```
...
Now that we have mastered some basic R knowledge, let's try to gain some insight into the safety of different states in the context of gun murders.
...
However, this does not give us information about which states have murder totals. For example, we don't know which state had 1257.
...
and `which.mas` for the index of the largest value:
...
For the minimum, we can use `min` and `whcih.min` in the same way.
...
Another common source of unnoticed errors in R is the use of *recycling*. We saw that vectors are added elementwise. So if the vectors don't match in length, it is natural to assume that we should get an error. But we can't. Noticed what happens:
```{r}
x <- c(1,2,3)
y <- c(10,20,30,40,50,60,70)
x+y
```
We should get warning, but no error. For the output, R has recycled the numbers in `x`. Notice that last digit of numbers in the output. 

<br/>

## 2.10 Exercise

For these exercise we will use the US murders dataset. Make sure you load it prior to starting.
```{r}
library(dslabs)
data("murders")
```
...
2. Now instead of the smallest population size, fine the index of the entry with the smallest population size. Hint: use `order` instead of `sort`.
```{r}
pop <- murders$population
order(pop)[1]
```
...
4. Now we know how small the smallest state is and we know which row represent it. Which state it is? Define a variable `states` to be the state names from the `murders` data frame. Report the name of the state with the smallest population.
```{r}
murders$state[51]
```
...
```{r}
temp <- c(35,88,42,84,81,30)
city <- c("Beiging", "Lagos", "Paris", "Rio de Janeiro", "San Juan", "Toronto")
city_temps <- data.frame(name=city, temperature=temp)
```

Use `rank` function to determine the population rank of each state from smallest population size to biggest. Save these ranks in an object called `ranks`, then create a data frame with the state name and its rank. Call the data frame `my_df`.
...
In R, arithmetic operators on vectors occur *element-wise*. For a quick example, suppose we have height in inches:
```{r}
inches <- c(69,62,66,70,73,67,73,67,70)
```
and want to convert to centimeters. Notice what happens when we multiply `incehs` by 2.54:
```{r}
inches*2.54
```
...
If we have two vectors of the same length, and we sum them in R, they will be added entry by entry. The same holds for other mathematical operations, such as `-`, `*` and `/`.

This implies that to compute the murder rates can simply type:
...
murder_rate <- murders$total/murders$population*100000
```
Imagine you are moving from Italy where, according to an ABC news report, the murder rate is only 0.71 per 100,000. You might would prefer to move to a state with a similar murder rate. Another powerful feature of R is that we can use logicals to index vectors. If we compare a vector to a singe number, it actually performs the test for each entry. The following is an example related to the question above:
```{r}
...
ind <- murder_rate<=0.71
```
Note that we get back a logical vector with `TRUE` for each entry smaller than or equal to 0.71. To see which state these are, we can leverage the fact vectors can be indexed with logicals.
```{r}
murders$state[ind]
```
In order to count how many TRUE, the function `sum` returns of the entries of a vector and logical vectors get *coerced* to numeric with `TRUE` coded as 1 and `FALSE` as 0. Thus we can count the states using:
```{r}
sum(ind)
```
...
```
```{r}
names(murder_rate) <- murders$state
...
If instead of just one state we want to find out the murder rates for several states, say New York, Florida, and Texas, we can use the function `match`. This function tells us which indexes of a second vector match each of the entries of a first vectors. 
```{r}
...
which(c("New York", "Florida", "Texas")%in%murders$state)
```
<br/>
...
2. Now use the results from the previous exercise and function `which` to determine the indices of `murder_rate` associated with values lower than 1. 
...
4. Now extent the code from exercises 2 and 3 to report the states in the Northeast with murder rates lower than 1. Hint: use the previously defined logical vector `low` and the logical operator `&`.
```{r}
ne <- murders$region == "Northeast"
murders$state[which(ne & low)]
```
...
7. Use the `%in%` operator to create a logical vector that answer the question: which of the following are actual abbreviations: MA, ME, MI, MO, MU?
```{r}
Ms <- c("MA", "ME", "MI", "MO", "MU")%in%murders$abb
Ms
```
...
Keep in mind that many states have populations below 5 million and are bunched up. We may gain further insight from making this plot in the log scale. Transform the variables using the `log10` transformation and then plot them.
```{r}
...
```{r}
boxplot(log10(population_in_millions) ~ region, data = murders)
```
